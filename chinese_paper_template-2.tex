...
\textbf{规则 2：} 符号中的下标 $k$ 为设备编号（如 PFM1-3、ICM2-5），下标 $j$ 为工序编号（如 A1、C5_3），下标 $i$ 为车间编号（如 A、B），下标 $g$ 为班组编号（1 或 2）。全文始终统一使用该编号约定。
...
设备编号格式为 $设备类型$组别$编号，如 PFM1-3 表示班组 1 的第 3 台精密灌装机。设备的初始位置为其所属班组的基地位置：班组 1 的设备初始位置在班组 1 基地，班组 2 的设备初始位置在班组 2 基地。每台设备具有确定的移动速度，同类型设备速度一致，具体取值从附件中获取。
