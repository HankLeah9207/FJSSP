...
# ═══════════════════════════════════════════════════════════════════════
# DATA — hardcoded from Q3_deepseek_problem_formulation.md
# Two-crew pool: Crew 1 (16 units) + Crew 2 (16 units) = 32 units total
# ═══════════════════════════════════════════════════════════════════════
...
for _etype, _units in EQUIPMENT_UNITS.items():
    for _uid in _units:
        ...
        # uid format e.g. "PFM1-3" or "ACA2-1"; the digit before '-' is crew
        UNIT_CREW[_uid] = int(_uid.split("-")[0][-1])
...
# Crew-specific initial travel time from each crew base to each workshop
# (seconds, from Table 2.3 of Q3 formulation)
INIT_TRAVEL = {
    1: {"A": 200, "B": 310, "C": 230, "D": 355, "E": 200},
    2: {"A": 250, "B": 230, "C": 310, "D": 340, "E": 275},
}

# Inter-workshop travel times (seconds), symmetric, identical for both crews.
_RAW_TRANSPORT = {
...
# ═══════════════════════════════════════════════════════════════════════
# MODEL — two-crew asynchronous operation-level CP-SAT formulation
# ═══════════════════════════════════════════════════════════════════════
...
def main():
    ...
    # Horizon: sum of all operation durations + travel buffer.
    # Use the maximum over inter-workshop and base-to-workshop travel times.
    ...
    # ── Assignment variables (two-crew pool, no crew restriction) ─────
    # assign[(pid, etype, uid)] = 1 iff unit uid serves operation (pid, etype).
    # Cross-crew sharing is permitted: a process needing two equipment
    # types may be served by units from different crews.
    assign = {}
    for pid in all_pids:
    ...
    # enforces the unit's own crew base→workshop travel time
    # (INIT_TRAVEL[crew(uid)][workshop]); node→node enforces inter-
    # workshop travel using early equipment release (operation end,
    # not PCT).
    ...
    for uid in ALL_UNITS:
        ...
        # SOURCE -> node i  (crew-specific initial travel)
        ...
        # node i -> node j  (i != j)  — early release: use op end, not PCT
        for i in range(1, n + 1):
            pi, ti = node_op[i]
        ...
        # AddCircuit 要求闭合回路：人工 SINK->SOURCE 弧将 SOURCE→...→SINK 路径闭环。
    ...
    # ── Model summary ─────────────────────────────────────────────────
    n_ops = sum(len(proc_info[pid]["equipment"]) for pid in all_pids)
    print("Solving Question 3 (Crew 1 + Crew 2, Workshops A-E) ...")
    print("Two-crew asynchronous operation-level model "
          "with early equipment release.")
    print(f"Processes: {len(all_pids)}")
    print(f"Operations: {n_ops}")
    print(f"Equipment units: {len(ALL_UNITS)} "
          f"(Crew 1: {sum(1 for u in ALL_UNITS if UNIT_CREW[u] == 1)}, "
          f"Crew 2: {sum(1 for u in ALL_UNITS if UNIT_CREW[u] == 2)})")
    ...
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        print("\nDiagnostic:")
        print(f"  - Horizon used: {horizon} s")
        print(f"  - Total processing time: {total_op} s")
        print(f"  - Bottleneck types (2 units total each): ASM, HPM")
        print("  - Likely cause: solver time limit reached or "
              "model is over-constrained for the chosen horizon.")
    ...
    print(f"Objective lower bound: {lb:.0f} seconds "
          f"({seconds_to_hms(int(lb))})")
    ...
    # Sort by Crew, then Equipment ID, then Start
    rows.sort(key=lambda r: (r["crew"], r["equipment_id"], r["start"]))

    # ── Validation (run before saving any output files) ───────────────
    all_pass = validate_schedule(rows, proc_info, all_pids)
    if not all_pass:
        print("\n*** Validation failed — output files not written. ***")
        return

    # ── Print Table 3 ─────────────────────────────────────────────────
    print(f"\nTable 3: Equipment Operation Schedule ({len(rows)} rows)")
    sep = "-" * 120
    print(sep)
    hdr = (f"{'No.':<5} {'Equipment ID':<12} {'Start Time':<12} "
           f"{'End Time':<12} {'Duration (s)':<14} "
           f"{'Process ID':<10} {'Crew'}")
    print(hdr)
    print(sep)
    for idx, r in enumerate(rows, 1):
        print(
            ...
            f"{r['duration']:<14} {r['process_id']:<10} {r['crew']}"
        )
    print(sep)
    ...
    with open(md_path, "w", encoding="utf-8") as f:
        f.write("# Question 3 Solution Summary\n\n")
        f.write("Two-crew asynchronous operation-level model with early "
                "equipment release. Crew 1 and Crew 2 share a unified "
                "32-unit pool; cross-crew sharing within a single process "
                "is permitted.\n\n")
        f.write(f"- Solver status: {status_name}\n")
        f.write(f"- Best makespan: {makespan} s "
                f"({seconds_to_hms(makespan)})\n")
        f.write(f"- Objective lower bound: {lb:.0f} s "
                f"({seconds_to_hms(int(lb))})\n")
        ...
        f.write("Rows belonging to the same process may have different "
                ...
                "of their End Times. Each unit is released at its own "
                "End Time (early release).\n\n")
...
# ═══════════════════════════════════════════════════════════════════════
# VALIDATION — two-crew asynchronous operation-level
# ═══════════════════════════════════════════════════════════════════════

def validate_schedule(rows, proc_info, all_pids):
    print("\n" + "=" * 60)
    print("SCHEDULE VALIDATION (two-crew asynchronous operation-level)")
    print("=" * 60)
    all_pass = True
    ...
    # 5. No overlapping operations per equipment unit, with travel
    unit_ops = {}
    for r in rows:
    ...
    for uid, ops in unit_ops.items():
        ...
        for i in range(len(ops_s) - 1):
            ...
            if gap < travel:
                print(
                    f"[FAIL] 5b. {uid}: {ops_s[i]['process_id']}({ws_c})"
                ...
                ok_travel = False
    if ok_overlap:
        print("[PASS] 5a. No overlapping operations on any equipment unit.")
    else:
        all_pass = False
    if ok_travel:
        print("[PASS] 5b. Consecutive ops respect inter-workshop travel "
              "(early-release based on operation end).")
    else:
        all_pass = False

    # 6. First op of each unit respects crew-specific base travel
    ok = True
    for uid, ops in unit_ops.items():
        ...
        if first["start"] < travel:
            print(f"[FAIL] 6. {uid} (Crew {crew}): first op "
                  f"{first['process_id']} starts {first['start']}s "
                  f"< base->{ws} {travel}s")
            ok = False
    if ok:
        print("[PASS] 6. All first operations respect crew-specific "
              "base travel time.")
    else:
        all_pass = False

    # 7. Early release endpoint — end == start + duration (definitional)
    ok = True
    for r in rows:
        if r["end"] != r["start"] + r["duration"]:
            print(f"[FAIL] 7. {r['equipment_id']} on {r['process_id']}: "
                  f"end mismatch")
            ok = False
    if ok:
        print("[PASS] 7. Each row's End = Start + Duration "
              "(no artificial delay to PCT).")
    else:
        all_pass = False

    # 8. Early release benefit analysis (informational)
    ...
    for uid, ops in unit_ops.items():
        ...
        for op in ops_s:
            ...
            if op["end"] < pct:
                benefit = pct - op["end"]
                print(
                    f"  {uid} (Crew {UNIT_CREW[uid]}): released from "
                    f"{op['process_id']} at {seconds_to_hms(op['end'])} "
                    f"(process ends {seconds_to_hms(pct)}, "
                    f"saved {benefit}s = {seconds_to_hms(benefit)})"
                )
                early_count += 1
    if early_count > 0:
        print(f"[INFO] 8. Early release used {early_count} time(s).")
    else:
        print("[INFO] 8. No early release benefit observed.")

    # 9. Cross-crew sharing analysis (informational)
    cross_crew = 0
    for pid in all_pids:
        prows = [r for r in rows if r["process_id"] == pid]
        if len(prows) >= 2:
            crews = set(r["crew"] for r in prows)
            if len(crews) > 1:
                cross_crew += 1
                print(f"  {pid}: cross-crew sharing — "
                      + ", ".join(f"{r['equipment_id']}(Crew {r['crew']})"
                                  for r in prows))
    print(f"[INFO] 9. Cross-crew sharing used in {cross_crew} process(es).")
